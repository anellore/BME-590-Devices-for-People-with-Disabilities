...
void shine_led() {
    analogWrite(PWM_LED, PWM_OUT); //value comes from mode set by operating modes
}


void flash_the_light() {
    analogWrite(PWM_LED, PWM_MAX);   // turn the LED on (HIGH is the voltage level)
    delay(1000/FLASH_RATE_HZ);                         // wait for a second
    analogWrite(PWM_LED, 0);        // turn the LED off by making the voltage 0 (if this doesn't work try "LOW")
    delay(1000/FLASH_RATE_HZ);                       // wait for a second
}
